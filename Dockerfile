...
# Pull base image.
FROM jlesage/baseimage-gui:ubuntu.20.04-v3.6.1  
...
RUN \
    export ROMVAULT_VERSION="$(grep romvault /VERSIONS | cut -d' ' -f2-)" && \
    ...
    set-cont-env WINDOW_NAME "RomVault ($ROMVAULT_VERSION) \/config" && \
